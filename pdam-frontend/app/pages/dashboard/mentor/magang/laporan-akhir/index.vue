...
const { laporanAkhirByMentor, pendingLaporanAkhirByMentor, refreshlaporanAkhirByMentor, pendingClearCache, clearCachelaporanAkhirByMentor,updateValidasiLaporanByMentor } = useLaporanAkhirByMentor(currentPage)


const breadcrumb = [
    { label: "Verifikasi Laporan Akhir", icon: "mdi:file-document-alert-outline",to: "/dashboard/mentor/magang/laporan-akhir" }
]
function goToPage(page: number) {
    if (page < 1 || page > (laporanAkhirByMentor.value?.data.last_page ?? 1)) return;
...
const validasiLaporanSchema = yup.object({ 
    status_verifikasi_mentor: yup.string().required('Status  wajib diisi'), 
})
const updateValidasiLaporan: SubmissionHandler<any> = async (value, { resetForm }: { resetForm: () => void }) => {
    const formData = new FormData()  
    formData.append('_method', 'PUT') 
    formData.append('id', value.id)
    formData.append('status_verifikasi_mentor', value.status_verifikasi_mentor)  
    if (value.status_verifikasi_mentor === 'rejected' && rejectionNote.value.trim()) {
        formData.append('rejection_note_mentor', rejectionNote.value);
    }
    ...
    try {
        ...
        const response = await updateValidasiLaporanByMentor(formData) 
        resetForm();
        addNotification('success', response.message) 
        await clearCachelaporanAkhirByMentor() 
...
<template>
    <NuxtLayout>
        ...
        <div class="bg-gray-50">
            <section class="mb-3">
                <div v-if="pendingLaporanAkhirByMentor" class="relative w-full h-[2px] bg-gray-200 overflow-hidden">
                    <div class="absolute h-full bg-gradient-to-r from-green-400 to-green-600 animate-loading"></div>
                </div>
                <div v-else class="p-3">
                    <div class="flex flex-row justify-between items-center mb-2">
                        <!-- <h3 class="text-md text-gray-700 font-medium">Verifikasi Laporan Akhir</h3> -->
                        <!-- <button @click="clearCachelaporanAkhirByMentor()" class="ms-auto">
                    ...
                    <div v-if="!laporanAkhirByMentor?.data?.data || laporanAkhirByMentor.data.data.length === 0"
                        ...
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    ...
                                    <tr v-for="data in laporanAkhirByMentor.data.data" :key="data.id"
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="flex flex-row flex-nowrap space-x-2 items-center">
                                                ...
                                                <span
                                                    ...
                                                    :class="{
                                                        'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300': data.status_verifikasi_mentor === 'approved',
                                                        'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300': data.status_verifikasi_mentor === 'pending',
                                                        'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300': data.status_verifikasi_mentor === 'rejected',
                                                    }">
                                                    <Icon :name="getIconName(data.status_verifikasi_mentor)"
                                                ...
                                                <div>
                                                    <Field as="select" name="status_verifikasi_mentor" id="status_verifikasi_mentor" v-model="selectedStatus"
                                                        ...
                                                        <option value="pending" :selected="data.status_verifikasi_mentor === 'pending'">
                                                            Tunggu</option>
                                                        <option value="approved" :selected="data.status_verifikasi_mentor === 'approved'">
                                                            Terima</option>
                                                        <option value="rejected" :selected="data.status_verifikasi_mentor === 'rejected'">
                                                            Tolak</option>
                                                    </Field>
                                                    <ErrorMessage name="status" class="text-red-500 text-xs"  />
                                                </div>
                                                <Field name="rejection_note_mentor" type="hidden" id="rejection_note_mentor" :value="rejectionNote" v-if="selectedStatus === 'rejected'"/>
                                                    ...
                                                    <div v-if="showRejectionModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50 ">
                                                        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg w-96">
                                                            ...
                                                            <textarea v-model="rejectionNote" rows="3" name="rejection_note_mentor" class="w-full mt-2 p-2 border border-gray-300 rounded-lg dark:bg-gray-700 dark:text-gray-200"></textarea>
                        ...
                        <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4 px-6 py-3"
                            ...
                            <span
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">{{
                                    laporanAkhirByMentor.data.from }}</span> -
                                <span class="font-semibold text-gray-900 dark:text-white">{{
                                    laporanAkhirByMentor.data.to }}</span> dari
                                <span class="font-semibold text-gray-900 dark:text-white">{{
                                    laporanAkhirByMentor.data.total }}</span>
                            </span>
                            ...
                            <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                                ...
                                <!-- Nomor Halaman -->
                                <li v-for="link in laporanAkhirByMentor.data.links" :key="link.label">
                                ...
                                <li>
                                    <button @click="goToPage(currentPage + 1)"
                                        :disabled="currentPage >= laporanAkhirByMentor.data.last_page"
                                        class="flex items-center justify-center px-3 h-8 leading-tight border rounded-e-lg transition"
                                        :class="currentPage < laporanAkhirByMentor.data.last_page ?
                                            'text-gray-500 bg-white border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
                                            : 'bg-gray-300 text-gray-500 cursor-not-allowed'">
                                        Next
                                    </button>
