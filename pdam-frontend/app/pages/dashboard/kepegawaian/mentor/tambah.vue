...
const { createUser, pendingCreate, errors, refresh } = useUsers(currentPage, 'mentor');
...
const breadcrumb = [
    {label: "User Mentor" , icon: "clarity:users-solid",  to: "/dashboard/kepegawaian/mentor", }, 
    {label: "Tambah" ,  }
]

const simpleSchema = yup.object({
    nisn_npm_nim_npp: yup.string().required('NPP wajib diisi(isikan strip jika belum mempunyai)'),
...
const handleCreate: SubmissionHandler<any> = async (value, { resetForm }: { resetForm: () => void }) => {
    try {
        ...
        await refresh(); 
        router.push('/dashboard/kepegawaian/mentor')
    } catch (error: any) { 
                    ...
                    <Form @submit="handleCreate" :validation-schema="simpleSchema"  class=" mx-auto">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                            ...
                            <div>
                                ...
                                <Field name="nisn_npm_nim_npp" type="number" id="nisn_npm_nim_npp" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                            ...
                            <div>
                                ...
                                <Field name="konfirmasi_password" type="password" id="konfirmasi_password" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                                <ErrorMessage name="konfirmasi_password" class="text-red-500 text-sm"/>
                                <p v-if="errors.konfirmasi_password" class="text-red-500 text-sm">{{ errors.konfirmasi_password[0] }}</p>
                            </div> 
                        </div> 
