...
const breadcrumb = [
    { label: "User Magang", icon: "clarity:users-solid", to: "/dashboard/kepegawaian/magang" }
]
 
const { users, pending, pendingToggle, toggleStatusUser, clearCacheUser, pendingClearCache } = useUsers(currentPage, 'magang')
                                    ...
                                    <tr v-for="user in users.data.data" :key="user.id"
                                        ...
                                        <td class="px-6 py-4">
                                            <p class="text-sm line-clamp-3 break-words">
                                                {{ user.alamat }}, {{ user.kabupaten_kota }}, {{ user.provinsi }} - {{
                                                    user.kode_pos }}
                                            </p>
                                        </td>
