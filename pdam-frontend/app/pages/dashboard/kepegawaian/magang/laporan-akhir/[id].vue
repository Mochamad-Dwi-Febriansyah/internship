...
const breadcrumb = [
    { label: "Dashboard", icon: "material-symbols:dashboard", to: "/dashboard/user" },
...
const report = ref({
    ...
    status: 'pending',
    deskripsi: 'Laporan sedang dalam tahap verifikasi kepegawaian.'
})
...
const { getLaporanAkhirById, pendingLaporanAkhirById } = useLaporanAkhirByKepegawaian(currentPage)
        ...
        <p class="text-gray-600 dark:text-gray-400">
          <span :class="statusColor(report.status)">{{ report.status }}</span>
                            ...
                            <div class="flex flex-col sm:col-span-2">
                                <p class="font-medium text-gray-700 dark:text-gray-300">Status Laporan Akhir</p>
                                <p class="text-gray-600 dark:text-gray-400">{{ report.deskripsi }}</p>
                            </div>
                        </div>
